...
class Expr:
    ...
    def __init__(self, op, *args):
        ...
        if not args and not is_prop_symbol(self.op):
            raise SyntaxError(
                "Unacceptable symbol base name (%s). Name must start with an upper-case alphabetic character that and is not TRUE or FALSE." % self.op)
...
class PropSymbolExpr(Expr):
    ...
    def __init__(self, sym_str, *index):
        ...
        if not is_prop_symbol(sym_str):
            raise SyntaxError(
                "Unacceptable symbol base name (%s). Name must start with an upper-case alphabetic character that and is not TRUE or FALSE." % sym_str)
        ...
        if len(index) == 0:
            ...
            raise SyntaxError(
                "Too many arguments to SymbolExpr constructor. SymbolExpr(symbol_str, [index1], [index2], [index3]")
