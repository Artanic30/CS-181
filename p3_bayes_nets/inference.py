...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for el_var in eliminationOrder:
            factors, result_factor = joinFactorsByVariable(factors, el_var)

            if not len(result_factor.unconditionedVariables()) == 1:
                factors.append(eliminate(result_factor, el_var))
        factors = joinFactors(factors)
