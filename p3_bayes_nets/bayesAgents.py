...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsStr = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(obsStr)
            variableDomainsDict[obsStr] = OBS_VALS
            for h in HOUSE_VARS:
                edges.append((h, obsStr))
    ...
    for housePos in [bottomLeftPos, topLeftPos, bottomRightPos, topRightPos]:
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                if housePos == eval(assignment['foodHouse'] + "Pos"):
                    if assignment[obs] == "red":
                        prob = PROB_FOOD_RED
                    elif assignment[obs] == "blue":
                        prob = 1 - PROB_FOOD_RED
                    else:
                        prob = 0
                elif housePos == eval(assignment['ghostHouse'] + "Pos"):
                    if assignment[obs] == "red":
                        prob = PROB_GHOST_RED
                    elif assignment[obs] == "blue":
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    result = inference.inferenceByVariableElimination(bayesNet, "FoodHouse", evidence, eliminationOrder)
    # print(result)
    max_pro = 0
    result_direction = None
    for ass in result.getAllPossibleAssignmentDicts():
        pro = bn.Factor.getProbability(result, ass)
        if pro > max_pro:
            max_pro = pro
            result_direction = ass
